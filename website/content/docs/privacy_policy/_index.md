...
To the extent that logsquirl app and website can provide their functionality without doing so, we prefer to avoid collecting data from you.
...
### Update Checking
By default, logsquirl periodically check to see if a newer version of the app is available, so that you can be given the choice to update if you wish.

An update check request is done by downloading file from logsquirl Github repository. This request does not contain any data.
...
### Data Not Collected
Except as described above, and as required to perform the application's core functionality at the user's request, logsquirl does not send out any private information. This includes:
