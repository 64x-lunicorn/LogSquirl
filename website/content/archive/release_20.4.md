...
During this months there were almost no new commits to logsquirl. For several years logsquirl has been my second most frequently used tool (first one being code editor) and I've been doing my best to improve it based on my daily experience and user feedback on github. However, since the beginning of 2020 I've switched to a more manager-like position. This is all very new to me, and doing this job requires tools not related to log file analysis. So I decided to make a release of logsquirl now to bring important fixes and new features to users of stable version. 

I will continue to work on logsquirl, but less frequent commits are expected. 

### Changelog

Firstly, logsquirl on MacOS is once again usable. Thanks for all users who reported problems:

 - MacOS dmg has more user friendly UI.
 - Fixed bug with saving user configuration.
 - Fixed bug with opening log files with logsquirl from Finder.
...
Major new features:
 ...
 - Open archives. LogSquirl can now open zip/gzip/bzip2/xz/7z archives. Files are extracted to temp directory.
 - Open files from network url. LogSquirl will download file to temp directory.
 - Multi-window mode. Allows to open several logsquirl windows. Still experimental feature. 
