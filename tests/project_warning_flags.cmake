...
# 13 and 14 printed -Wstringop-overflow on every appimage, noble and oracle run
# and everything stayed green. What holds that shut now is one flag on the link
...
# it is accepted for the GCC versions that report it, and it has to stop being
# accepted by itself once the oldest GCC LogSquirl is built with is newer than
# those. Here the compiler and its version are arguments, so each of those
# answers can be read back without having that compiler -- which is how the
# bound below covers GCC 14, a version the ticket did not know reported it.
#
...
set(GCC12 COMPILER_ID GNU COMPILER_VERSION 12.3.0 IS_MSVC OFF AS_ERRORS ON SANITIZERS OFF)
set(GCC13 COMPILER_ID GNU COMPILER_VERSION 13.2.0 IS_MSVC OFF AS_ERRORS ON SANITIZERS OFF)
set(GCC14 COMPILER_ID GNU COMPILER_VERSION 14.3.1 IS_MSVC OFF AS_ERRORS ON SANITIZERS OFF)
set(GCC15 COMPILER_ID GNU COMPILER_VERSION 15.2.0 IS_MSVC OFF AS_ERRORS ON SANITIZERS OFF)
set(GCC16 COMPILER_ID GNU COMPILER_VERSION 16.0.1 IS_MSVC OFF AS_ERRORS ON SANITIZERS OFF)
set(APPLECLANG COMPILER_ID AppleClang COMPILER_VERSION 17.0.0 IS_MSVC OFF AS_ERRORS ON SANITIZERS OFF)
# COMPILER_ID is spelled out as "MSVC" here on purpose: that is what
# CMAKE_CXX_COMPILER_ID really is on Windows, and it is why the keyword next to
# it is IS_MSVC rather than MSVC. With both called MSVC, cmake_parse_arguments()
# read the value as the keyword and left COMPILER_ID unset, with a CMake author
# warning on every Windows configure to show for it.
set(MSVC2022 COMPILER_ID MSVC COMPILER_VERSION 19.44.35207.1 IS_MSVC ON AS_ERRORS ON SANITIZERS OFF)
...
set(GCC13_LENIENT COMPILER_ID GNU COMPILER_VERSION 13.2.0 IS_MSVC OFF AS_ERRORS OFF SANITIZERS OFF)
expect_flag(link LACKS -Werror OF ${GCC13_LENIENT})
expect_flag(compile LACKS -Werror OF ${GCC13_LENIENT})

# The accepted warning, and its expiry. Every GCC that was measured reporting
# -Wstringop-overflow for a write that cannot go past anything is covered --
# the appimage's 12, noble's 13 and oracle's 14.3.1 -- and so is 15, which no
# job here builds and which is therefore covered rather than guessed at. GCC 16
# was measured clean, and by then the flag is gone rather than carried along.
expect_flag(link HAS -Wno-error=stringop-overflow OF ${GCC12})
expect_flag(link HAS -Wno-error=stringop-overflow OF ${GCC13})
expect_flag(link HAS -Wno-error=stringop-overflow OF ${GCC14})
expect_flag(link HAS -Wno-error=stringop-overflow OF ${GCC15})
...
set(GCC13_SANITIZED COMPILER_ID GNU COMPILER_VERSION 13.2.0 IS_MSVC OFF AS_ERRORS ON SANITIZERS ON)
