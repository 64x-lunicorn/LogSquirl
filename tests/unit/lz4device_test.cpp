...
/// Compresses raw data into one complete LZ4 frame.
QByteArray lz4Frame( const QByteArray& rawData )
{
    const auto maxDst
        = LZ4F_compressFrameBound( static_cast<std::size_t>( rawData.size() ), nullptr );
...
/// Writes bytes as they are to a temporary .lz4 Log File.
bool writeRawFile( QTemporaryFile& file, const QByteArray& bytes )
{
    file.setFileTemplate( "lz4_log_XXXXXX.lz4" );
    if ( !file.open() ) {
        return false;
    }
...
SCENARIO( "A .lz4 Log File made of several frames decompresses completely",
          "[lz4device][decompressor]" )
{
    GIVEN( "A Log File whose Log Lines are split over three concatenated frames" )
    {
        ...
        const auto frames = lz4Frame( firstPart ) + lz4Frame( secondPart ) + lz4Frame( thirdPart );
        ...
        WHEN( "the frames are read through an Lz4Device in small pieces" )
        {
            Lz4Device device( logFile.fileName() );
            REQUIRE( device.open( QIODevice::ReadOnly ) );
...
SCENARIO( "A truncated .lz4 Log File reports an error", "[lz4device][decompressor]" )
{
    GIVEN( "A two-frame Log File cut off in the middle of its second frame" )
    {
        const auto firstFrame = lz4Frame( logLines( 1, 1000 ) );
        const auto secondFrame = lz4Frame( logLines( 1001, 20000 ) );
    ...
    GIVEN( "A single-frame Log File missing only its end mark" )
    {
        const auto frame = lz4Frame( logLines( 1, 5000 ) );
...
SCENARIO( "A corrupt .lz4 Log File reports an error", "[lz4device][decompressor]" )
{
    GIVEN( "A Log File whose valid first frame is followed by garbage" )
    {
        const auto corrupt = lz4Frame( logLines( 1, 1000 ) ) + QByteArray( 64, 'x' );
