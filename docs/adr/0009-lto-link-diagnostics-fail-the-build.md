...
That is not hypothetical. The appimage (GCC 12), noble (GCC 13) and oracle (GCC 14) jobs printed, on every run:
...
GCC 16 (the fedora job) does not report it.

**And a third job reports it that the ticket did not know about.** The first CI run of this change turned the oracle job red, on GCC 14.3.1, with the same two lines — it had been printing them all along, unread, exactly as the appimage and noble jobs had. That is the change doing its job on its first run, and it is the reason the guard covers 14 as well. The full inlining chain the failure printed is worth keeping:

```
In member function 'setPresentationPolicy',
    inlined from 'setPresentationPolicy' at src/ui/src/abstractlogview.cpp:1277:37,
    inlined from 'setPresentationPolicy' at src/ui/src/logmainview.cpp:120:43,
    inlined from 'seed' at src/ui/src/viewset.cpp:197:40,
    inlined from 'addPresentation' at src/ui/src/viewset.cpp:40:9,
    inlined from 'setup' at src/ui/src/crawlerwidget.cpp:1422:29:
src/textviewscrolling/src/textviewscrolling.cpp:319:25: error: writing 13 bytes into a region of size 0
src/ui/include/logmainview.h:48:7: note: at offset -856 into destination object '<anonymous>' of size 8
```

Five levels of cross-translation-unit inlining, and the destination object GCC ends up describing is `'<anonymous>' of size 8` — the `LogPresentation` base subobject's bare vptr. The second diagnostic names offset **176**, a positive one, into the same "size 8" object: whichever direction the arithmetic goes, the object GCC is measuring against is the interface subobject and not the complete one.

GCC 15 is built nowhere here, so it is not measured either way. It is covered rather than left out: a `-Wno-error=` for a diagnostic the compiler does not emit costs nothing, while a version left uncovered costs a red build on the day an image is bumped. The bound moves down when a job measures 15 clean, not before.

**Conclusion: a GCC bug in the late interprocedural passes, not a finding about LogSquirl's code.** There is no write to correct and no member to move.
...
- **The `-Wstringop-overflow` is recorded as an accepted warning, with an expiry.** `-Wno-error=stringop-overflow` goes on the link line for `GNU` below version 16, and nowhere else. It is deliberately *not* `-Wno-stringop-overflow`: the warning stays in the log, where the next person can see it is still there. And deliberately not a `#pragma GCC diagnostic` around the two setters: GCC raises this one from the LTRANS partition at link time, where the source file's pragma state is not reliably honoured, and a pragma would also put a workaround for a compiler bug into production code that has nothing wrong with it.
- **The expiry is a guard, not a reminder.** `ARG_COMPILER_VERSION VERSION_LESS 16` takes the flag back by itself once the oldest GCC LogSquirl is built with reaches 16; `project_warning_flags` asserts both halves, that 12, 13, 14 and 15 get it and that 16 does not. Note this is not close: the appimage job pins GCC 12 on purpose (`docker/ubuntu22.04/Dockerfile`), because it fixes the AppImage's glibc/libstdc++ floor at 2.35, so that pin will outlive several Ubuntu releases.

## Consequences

- A GCC warning raised at link time now fails the build, on all four Linux jobs at once — which is how the oracle job's copy of this diagnostic came to light on the very first run, and the sanitizer job is no exception (it builds with `-DLOGSQUIRL_USE_LTO=OFF`, so it has no link-time code generation to report on; its own accepted warning, `-Wno-error=maybe-uninitialized`, stays where it was, at compile time).
